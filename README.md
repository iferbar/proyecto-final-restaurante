# 🍴 GastroGuide - Cheat Sheet

### 🚀 Inicialización Rápida (Docker)
Desde la raíz del proyecto:
```bash
cd Backend && docker compose up -d
cd ../Frontend && npm install && npm run dev
```

### 🛠️ Estructura de Puertos
- **Frontend**: [http://localhost:5173](http://localhost:5173) (Vite)
- **API Backend**: [http://localhost:4000](http://localhost:4000)
- **phpMyAdmin**: [http://localhost:9091](http://localhost:9091)

### 📋 Requisitos
- Docker & Docker Compose
- Node.js (v18+)

### 💡 Comandos Útiles
- **Parar todo**: `docker compose down` (en Backend)
- **Limpiar CSS/Lint**: `npm run lint` (en Frontend)
- **Build Pro**: `npm run build` (en Frontend)


